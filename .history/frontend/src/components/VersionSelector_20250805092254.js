...
function VersionSelector() {
  ...
  const fetchPromptAndVersions = async () => {
    try {
      ...
      // 统一处理响应数据，确保它是一个数组
      let versionArray = [];
      if (Array.isArray(versionsResponse.data)) {
        versionArray = versionsResponse.data;
      } else if (versionsResponse.data && versionsResponse.data.results && Array.isArray(versionsResponse.data.results)) {
        versionArray = versionsResponse.data.results;
      }
      
      console.log('Processed version array:', versionArray);
      setVersions(versionArray);
      
      if (versionArray.length > 0) {
        setSelectedVersion(versionArray[0]);
      }
    } catch (err) {
  ...
  const handleRestore = async (versionId) => {
    try {
      const token = localStorage.getItem('token');
      const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:8000/api';
      await axios.post(`${API_BASE_URL}/prompts/${promptId}/versions/${versionId}/restore/`, {}, {
        headers: { Authorization: `Token ${token}` }
      });
            ...
            <List sx={{ p: 0 }}>
              {Array.isArray(versions) && versions.map((version, index) => (
                <React.Fragment key={version.id}>
                  ...
                  {Array.isArray(versions) && index < versions.length - 1 && <Divider />}
                </React.Fragment>
              ))}
            </List>
          </Paper>
