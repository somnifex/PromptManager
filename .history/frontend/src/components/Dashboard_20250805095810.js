...
function Dashboard() {
  ...
  const filteredPrompts = prompts.filter(prompt => {
    ...
    if (selectedTag && hasTags) {
      console.log('Prompt tags:', prompt.tags);
      console.log('Selected tag:', selectedTag);
    }
    
    const matchesTag = !selectedTag || (hasTags && prompt.tags.some(tag => tag.id === selectedTag.id));
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap">
                  ...
                  {(!Array.isArray(tags) || tags.length === 0) && (
                    <Typography variant="body2" color="text.secondary" sx={{ p: 1, fontStyle: 'italic' }}>
                      {t('dashboard.noTagsAvailable') || '暂无可用标签，您可以在创建或编辑提示词时添加标签'}
                    </Typography>
                  )}
                </Stack>
              ) : (
