...
export const createPrompt = createAsyncThunk(
  ...
  async ({ title, content, sharing_mode, commit_message, tag_ids }) => {
    const token = localStorage.getItem('token');
...
export const updatePrompt = createAsyncThunk(
  ...
  async ({ id, title, content, sharing_mode, commit_message, tag_ids }) => {
    const token = localStorage.getItem('token');
...
export const fetchAllTags = createAsyncThunk(
  'prompts/fetchAllTags',
  async () => {
    const token = localStorage.getItem('token');
    const response = await axios.get(`${API_BASE_URL}/prompts/tags/`, {
      headers: { Authorization: `Token ${token}` }
    });
    return response.data;
  }
